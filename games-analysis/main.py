...
plt.scatter(df_main["first_release_date"], df_main["critic_rating_value"], color = "blue", label = "Critic Raitings")
...
df_main = pd.read_csv("game-ratings-by-top-genres.csv")
...
df_hype = df_main.groupby(["genre_name"])["hype_count"].sum().reset.index()
...
plt.bar(df_hype.index + BAR_WIDTH / 2, df_hype["totalt_hypes"], color = "red", label = "Number of Hypes", width = BAR_WIDTH)
...
df_main = pd.read_csv("game-ratings-by-top-platforms.csv")

df_follow = df_main.groupby(["platform_name"])["follow_count"].sum().reset_index()

df_follow = df_follow.rename(columns = { "follow_count":"total_follows"})

df_hype = df_main.groupby(["platform_name"])["hype_count"].sum().reset.index()
